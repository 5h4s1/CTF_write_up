...
```
Bài này dùng thuật toán xor để mã hóa $defaultdata với 1 key secret, sau đó gắn vào COOKIE.

Mình viết 1 script để tìm ra key:
```php
<?php
...
var_dump(saveData($defaultdata));
```
Key: qw8J

Rồi chỉnh $defaultdata thành: array("showpassword" => "yes", "bgcolor" => "#ffffff") để check sau đó show ra password
...
Chạy script là ra cookie rồi thay đổi cookie trên web để lấy pass
